...
export function KeyTopicsSection({ topics }: KeyTopicsSectionProps) {
  return (
    <section className="mb-12">
      <h2 className="mb-2 text-xl font-semibold text-slate-900">Key Topics</h2>
        ...
        {topics.map((topic) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <h3 className="font-medium text-slate-900 transition-colors group-hover:text-sky-300">
                {topic.title}
              </h3>
              <p className="mt-0.5 text-xs text-slate-400">{topic.subjectName}</p>
            </div>
    ...
    <div className="flex flex-wrap gap-2">
      {topics.map((topic) => (
        <Link
          key={topic.href}
          href={topic.href}
          className="rounded-full border border-slate-200 bg-sky-400/5 px-3 py-1 text-sm text-slate-600 transition-all hover:border-orange-300 hover:bg-orange-500/10 hover:text-orange-300"
        >
          {topic.label}
        </Link>
      ))}
